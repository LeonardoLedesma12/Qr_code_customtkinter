...
class App(ctk.CTk):
    ...
    def save(self, event = ''):
        if self.raw_image:
            ...
            if file_path:
                self.raw_image.save(f'{file_path}.jpg')
            

class EntryField(ctk.CTkFrame):
    def __init__(self, parent, entry_string, save_qr):
        super().__init__(master = parent, fg_color = '#021fb3', corner_radius = 20)
...
class QrImage(ctk.CTkCanvas):      
    def __init__(self, parent):
        super().__init__(
            ...
            relief = 'ridge')
        self.place(relx = 0.5, rely = 0.4, height=200, width=200, anchor='center') 
